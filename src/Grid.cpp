...
float Grid::ComputeBendOffset(const Sphere& sphere, const glm::vec3& vertexPos, float G, float lightSpeed) {
    // Calculate the Schwarzschild radius for the sphere
    float rs = (2 * G * sphere.mass) / (lightSpeed * lightSpeed);
    float r = glm::length(vertexPos - sphere.position); // radial distance from sphere center

    float offset = 0.0f;
    if (r > rs) {
        // The constant factor (e.g. 4.0f * sqrt(rs * 1000.0f)) can be adjusted for visual scaling.
        offset = 4.0f * sqrt(rs * 1000.0f * (r - rs));
    }
    return offset;
}


void Grid::ApplyMultipleSphereDeformations(const std::vector<Sphere>& spheres, float G, float lightSpeed) {
    // Reset to original vertices before applying any offsets.
    vertices = originalVertices;

    // Optionally, determine a baseline vertical shift for the grid if needed.
    // Here, we use the y-value stored in originalVertices as baseY.
    for (size_t i = 0; i < vertices.size(); i += 3) {
        float baseY = originalVertices[i + 1];
        float totalOffset = 0.0f;
        glm::vec3 vertexPos(originalVertices[i], originalVertices[i + 1], originalVertices[i + 2]);

        // Sum offsets from every sphere.
        for (const auto& sphere : spheres) {
            totalOffset += ComputeBendOffset(sphere, vertexPos, G, lightSpeed);
        }



        // Update the y-coordinate of the vertex with the combined offset.
        vertices[i + 1] = baseY + totalOffset;
    }

    // Now update the GPU buffer so the rendered grid uses the new vertices.
    UpdateBuffer();
}
